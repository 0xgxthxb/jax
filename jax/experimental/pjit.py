...
from typing import Callable, Sequence, Tuple, Union, Optional, List, cast
...
def _prepare_axis_resources(axis_resources,
  ...
  entries = [
      entry if _is_from_gda(entry) or _is_auto(entry) else
...
def get_array_mapping(axis_resources: Union[ParsedPartitionSpec, _AUTOAxisResource]) -> pxla.AutoArrayMapping:
    ...
    if isinstance(arg, GDA):
      ...
      if in_cpspec != gda_cpspec:
        raise ValueError(
            ...
            "returned by `pjit.get_sharding_from_xla`? "
            "If yes, please file a bug. "
...
def _get_partition_spec(ppspec: Union[List[ParsedPartitionSpec], ParsedPartitionSpec]) -> Sequence[PartitionSpec]:
  if isinstance(ppspec, list):
    return [pxla.array_mapping_to_axis_resources(cast(pxla.ArrayMapping, get_array_mapping(p)))
            for p in ppspec]
  else:
    return [pxla.array_mapping_to_axis_resources(cast(pxla.ArrayMapping, get_array_mapping(ppspec)))]


def _get_ppspec_from_executable(executable, mesh):
  input_op_shardings = executable.hlo_modules()[0].spmd_parameters_shardings
  output_op_sharding = executable.hlo_modules()[0].spmd_output_sharding
  in_ppspec = [parse_op_sharding(sharding, mesh) for sharding in input_op_shardings]
...
def _get_sharding_from_executable(
  ...
  in_partition_spec: List[PartitionSpec] = []
  for spec in in_ppspec:
    in_partition_spec.extend(_get_partition_spec(spec))
...
def get_sharding_from_xla(pjitted_fn, mesh, global_shapes, dtypes):
  ...
  in_sharding, out_sharding = _get_sharding_from_executable(
      compiled.runtime_executable(), mesh)
  return in_sharding, out_sharding
